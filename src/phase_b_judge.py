...
def pairwise_judge(question: str, answer_a: str, answer_b: str) -> dict:
    ...
    if not OPENAI_API_KEY:
        return {"winner": "tie", "reasoning": "no API key configured", "scores": {"A": 0.0, "B": 0.0}}
...
'''
    ...
    try:
        ...
        parsed.setdefault("scores", {"A": 0.0, "B": 0.0})
        return parsed
...
if __name__ == "__main__":
    # --- Load 10 human-labeled questions + their ground_truth from the 50q test set ---
    ...
    print(f"Running swap-and-average judge on {len(human_data)} labeled questions "
          f"(A=model_answer vs B=ground_truth)...")
    for item in human_data:
        gt = test_set[item["question_id"]]["ground_truth"]
        result = swap_and_average(item["question"], item["model_answer"], gt)
        judge_results.append(result)
        human_labels.append(item["human_label"])
        # judge_label = 1 if the judge considers model_answer at least as good as ground_truth
        judge_labels.append(1 if result.final_winner in ("A", "tie") else 0)
        print(f"  Q{item['question_id']}: judge={judge_labels[-1]} human={item['human_label']} "
              f"(final_winner={result.final_winner}, position_consistent={result.position_consistent})")
    ...
    report = {
        ...
        "per_question": [
            ...
            for item, r, jl in zip(human_data, judge_results, judge_labels)
        ],
    }
